"""Confirms the independently-coded engine (scripts/independent_nq_strict_engine.py)
reproduces the canonical strict ledger (outputs/nq_strict_executed.csv) exactly,
...
def reference():
    return _read_with_ts(os.path.join(OUT_DIR, "nq_strict_executed.csv"), ["entry_time", "exit_time"])


@pytest.fixture(scope="module")
def independent():
    return _read_with_ts(os.path.join(OUT_DIR, "nq_independent_executed.csv"), ["entry_time", "exit_time"])
